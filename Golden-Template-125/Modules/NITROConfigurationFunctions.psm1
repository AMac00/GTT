...
<#
    ...
    # Created: 20160912
        ...
        .PARAMETER Hostname
            Domain name
        .PARAMETER IPAddress
            An IPv4 address to assign to the domain name
        .PARAMETER TTL
            Time to Live (TTL), in seconds, for the record
        .EXAMPLE
            New-DNSAddRec -NSSession $NSSession -Hostname $Hostname -IPAddress $IPAddress -TTL $TTL
        .NOTES
        ...
        param (
            ...
            [Parameter(Mandatory=$true)] [string]$Hostname,
            [Parameter(Mandatory=$true)] [string]$IPAddress,
            [Parameter(Mandatory=$false)] [double]$TTL
        )
        ...
        $payload = @{hostname=$Hostname;ipaddress=$IPAddress;ttl=$TTL}

        $response = Invoke-NSNitroRestApi -NSSession $NSSession -OperationMethod POST -ResourceType "dnsaddrec" -Payload $payload
        Write-Host $response
        Write-Verbose "$($MyInvocation.MyCommand): Exit"
    }
    ...
    # Created: 20160912
        ...
        .EXAMPLE
            New-Server -NSSession $NSSession -name $Name -ipaddress $IPAddress
        .NOTES
            Copyright (c). All rights reserved.
        #>
        [CmdletBinding()]
    ...
    # Created: 20160912
        ...
        .PARAMETER publicip
            The public IP address that a NAT device translates to the GSLB services private IP address
        .PARAMETER publicport
            The public port associated with the GSLB services public IP address
        .PARAMETER sitename
        ...
        .EXAMPLE
            New-GSLBService -NSSession $NSSession -servicename $ServiceName -servername $ServerName -ip $IP -servicetype $ServiceType -port $Port -publicip $PublicIP -publicport $PublicPort -sitename $SiteName -monitor_name_svc $MonitorName -gslb $GSLB
        .NOTES
            Copyright (c). All rights reserved.
        #>
        [CmdletBinding()]
        ...
        $payload = @{servicename=$servicename;servername=$servername;servicetype=$servicetype;port=$port;publicip=$publicip;ipaddress=$ipaddress;publicport=$publicport;sitename=$sitename}

        $response = Invoke-NSNitroRestApi -NSSession $NSSession -OperationMethod POST -ResourceType "gslbservice" -Payload $payload
        Write-Host $response
        Write-Verbose "$($MyInvocation.MyCommand): Exit"
    }

    function New-GSLBService_LBMonitor_Binding {
    # Created: 20160912
        <#
        .SYNOPSIS
            Bind a monitor to an existing GSLB Service
        .DESCRIPTION
            Bind a monitor to an existing GSLB Service
        .PARAMETER NSSession
            An existing custom NetScaler Web Request Session object returned by Connect-NSAppliance
        ...
        .EXAMPLE
            New-GSLBService_LBMonitor_Binding -NSSession $NSSession -servicename $ServiceName -monitor_name $MonitorName -weight $Weight
        .NOTES
            Copyright (c). All rights reserved.
        #>
        [CmdletBinding()]
        param (
            [Parameter(Mandatory=$true)] [PSObject]$NSSession,
            [Parameter(Mandatory=$true)] [string]$servicename,
            [Parameter(Mandatory=$true)] [string]$monitor_name,
            [Parameter(Mandatory=$true)] [double]$weight
        )

        Write-Verbose "$($MyInvocation.MyCommand): Enter"

        $payload = @{servicename=$servicename;monitor_name=$monitor_name;weight=$weight}

        $response = Invoke-NSNitroRestApi -NSSession $NSSession -OperationMethod POST -ResourceType "gslbservice_lbmonitor_binding" -Payload $payload
        Write-Host $response
        Write-Verbose "$($MyInvocation.MyCommand): Exit"
    }

    function New-GSLBVServer {
    # Created: 20160912
        <#
        .SYNOPSIS
            Create a new GSLB Virtual Server
        .DESCRIPTION
            Create a new GSLB Virtual Server
        .PARAMETER NSSession
            An existing custom NetScaler Web Request Session object returned by Connect-NSAppliance
        ...
        .EXAMPLE
            New-GSLBVServer -NSSession $NSSession -name $Name -servicetype $ServiceType -backupvserver $BackupVServer
        .NOTES
            Copyright (c). All rights reserved.
        #>
        [CmdletBinding()]
        ...
        $response = Invoke-NSNitroRestApi -NSSession $NSSession -OperationMethod POST -ResourceType "gslbvserver" -Payload $payload
        Write-Host $response
        Write-Verbose "$($MyInvocation.MyCommand): Exit"
    }

    function Update-GSLBVServer {
    # Created: 20160912
        <#
        .SYNOPSIS
            Update an existing GSLB Virtual Server
        .DESCRIPTION
            Update an existing GSLB Virtual Server
        .PARAMETER NSSession
            An existing custom NetScaler Web Request Session object returned by Connect-NSAppliance
        .PARAMETER name
            Name of the GSLB virtual server to update
        .PARAMETER backupvserver
            Name of the backup GSLB virtual server to which the appliance should to forward requests if the status of the primary GSLB virtual server is down or exceeds its spillover threshold
            
        .EXAMPLE
            Update-GSLBVServer -NSSession $NSSession -name $Name -backupvserver $BackupVServer
        .NOTES
            Copyright (c). All rights reserved.
        #>
        [CmdletBinding()]
        param (
            [Parameter(Mandatory=$true)] [PSObject]$NSSession,
            [Parameter(Mandatory=$true)] [string]$name,
            [Parameter(Mandatory=$false)] [string]$backupvserver
        )

        Write-Verbose "$($MyInvocation.MyCommand): Enter"

        if ($backupvserver) {
            $payload = @{name=$name;backupvserver=$backupvserver}
        }

        $response = Invoke-NSNitroRestApi -NSSession $NSSession -OperationMethod PUT -ResourceType "gslbvserver" -Payload $payload
        Write-Host $response
        Write-Verbose "$($MyInvocation.MyCommand): Exit"
    }

    function New-GSLBVServer_Domain_Binding {
    # Created: 20160912
        <#
        .SYNOPSIS
            Bind a new domain to a GSLB Virtual Server
        .DESCRIPTION
            Bind a new domain to a GSLB Virtual Server
        .PARAMETER NSSession
            An existing custom NetScaler Web Request Session object returned by Connect-NSAppliance
        ...
        .EXAMPLE
            New-GSLBVServer_Domain_Binding -NSSession $NSSession -name $Name -ttl $TTL -domainname $DomainName
        .NOTES
            Copyright (c). All rights reserved.
        #>
        [CmdletBinding()]
        param (
            [Parameter(Mandatory=$true)] [PSObject]$NSSession,
            [Parameter(Mandatory=$true)] [string]$name,
            [Parameter(Mandatory=$true)] [double]$ttl,
            [Parameter(Mandatory=$true)] [string]$domainname
        )

        Write-Verbose "$($MyInvocation.MyCommand): Enter"

        $payload = @{name=$name;ttl=$ttl;domainname=$domainname}

        $response = Invoke-NSNitroRestApi -NSSession $NSSession -OperationMethod POST -ResourceType "gslbvserver_domain_binding" -Payload $payload
        Write-Host $response
        Write-Verbose "$($MyInvocation.MyCommand): Exit"
    }

    function New-GSLBVServer_GSLBService_Binding {
    # Created: 20160912
        <#
        .SYNOPSIS
            Bind a GSLB Service to a GSLB Virtual Server
        .DESCRIPTION
            Bind a GSLB Service to a GSLB Virtual Server
        .PARAMETER NSSession
            An existing custom NetScaler Web Request Session object returned by Connect-NSAppliance
        .PARAMETER name
            Name of the virtual server on which to perform the binding operation
        .PARAMETER servicename
            Name of the GSLB service for which to bind

        .EXAMPLE
            New-GSLBVServer_GSLBService_Binding -NSSession $NSSession -name $Name -servicename $ServiceName
        .NOTES
            Copyright (c). All rights reserved.
        #>
        [CmdletBinding()]
        ...
        $payload = @{name=$name;servicename=$servicename}

        $response = Invoke-NSNitroRestApi -NSSession $NSSession -OperationMethod POST -ResourceType "gslbvserver_gslbservice_binding" -Payload $payload
        Write-Host $response
        Write-Verbose "$($MyInvocation.MyCommand): Exit"
    }

    function Get-GSLBAutoSyncState {
    # Created: 20160912
        <#
        .SYNOPSIS
            Checks whether GSLB Auto Sync is enabled
        .DESCRIPTION
            Checks whether GSLB Auto Sync is enabled
        .PARAMETER NSSession
            An existing custom NetScaler Web Request Session object returned by Connect-NSAppliance
        ...
        .EXAMPLE
            Get-GSLBAutoSyncState -NSSession $NSSession
        .NOTES
            Copyright (c). All rights reserved.
        #>
        [CmdletBinding()]
        ...
        $response = Invoke-NSNitroRestApi -NSSession $NSSession -OperationMethod GET -ResourceType "gslbparameter" 
        
        if ($response.gslbparameter.automaticconfigsync -eq "ENABLED") {
            return "ENABLED"
        } elseif ($response.gslbparameter.automaticconfigsync -eq "DISABLED") {
            return "DISABLED"
        } else {
            Write-Host $response
        }
